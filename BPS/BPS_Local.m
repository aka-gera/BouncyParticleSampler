...
function [t_skeleton, x_skeleton, v_skeleton]=BPS_Local(  x,v,fun,T, refresh_rate)
i1=2;
j1=3;
dim=size(x,1);
I=ones(dim,1);
...
while finished==0
    ...
    if ( Diff_t_switch_proposed < Diff_t_refresh)
        ...
        if rand* proposedSwitchIntensity <= switch_rate
            % reflect
            v(argMin)=  Reflect(Nabla(argMin),v(argMin));
...
% dlmwrite(['x',nom,'_H.txt'],x_skeleton','delimiter','\t','precision',5)
